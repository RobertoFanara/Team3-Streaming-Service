...
const Comico = () => {
  ...
  const [smallFilmEditing, setSmallFilmEditing] = useState(null);
      ...
      const api = 'https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=it-IT&page=1&region=IT&sort_by=popularity.desc&with_genres=35';
      ...
      <div className="flex item-center justify-center mt-10 text-[RGB(255,187,56)] pt-[50px] pb-[30px]">
        {smallFilmEditing && (
          <FilmDetails
            ...
            vote={
              smallFilmEditing.vote_average == 0
                ? "Ancora nessuna valutazione"
                : smallFilmEditing.vote_average
            }
        ...
        <div className="flex border border-black w-[870px] gap-5 flex-wrap">
                  ...
                  >
                    <div className="absolute hover:stroke-white stroke-[rgb(255,187,65)] transition duration-200 ease-in-out">
                      <svg
                        ...
                        strokeWidth={1.5}
                        stroke="currentColor"
                        className="absolute top-[210px] left-[60px] w-[35px] h-[35px] stroke-inherit z-20 cursor-pointer"
                      >
                        <path
                          strokeLinecap="round"
          ...
          <div className="w-[100%] flex items-center justify-center text-2xl space-x-2">
            <div>1</div>
            <Link to={"/comicoSecondPage"} onClick={topFunction}>
              <div>»</div>
            </Link>
          </div>
        </div>
